...
function isLocalFullSlot(track, slot) {
  if (track !== 'A') return false;
  if (typeof slot === 'string') return slot.includes('12:00-14:00');
  return String(slot?.time || '').trim() === '12:00-14:00';
}

function normalizeYesNo(value) {
  if (value === 'yes' || value === '是') return '是';
  if (value === 'no' || value === '否') return '否';
  return '否';
}
...
module.exports = async function handler(req, res) {
  ...
  if (!Array.isArray(body.slots) || !body.slots.length) return res.status(400).json({ ok: false, message: '請至少選擇一個流量扶持時段' });

  try {
    if (body.slots.some(function(slot) { return isLocalFullSlot(body.track, slot); })) {
      return res.status(400).json({ ok: false, message: '本地賽道 12:00-14:00 報名時間段已滿，請選擇其他時段' });
    }
    ...
    if (body.track === 'A') {
      ...
      const fields = { 'TikTok 使用者名稱': body.username, '申請時段': slotsText, '賽道': '海外流量', '審核狀態': '待審核', '掃碼授權完成': normalizeYesNo(body.overseasKycScanned), '願意與海外主播PK': normalizeYesNo(body.overseasWillingPk), 'LIVE Studio 權限': normalizeYesNo(body.overseasHasLiveStudio) };
      if (body.videoUrl) fields['推薦推流短影片連結'] = body.videoUrl;
      if (period) fields['申請期數'] = period;
